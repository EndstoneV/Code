...
bool co(int be, int en) {
    ...
    while(nn <= en) {
        if(lo[nn] == '<') {
            ...
            ++now;
            if(now > 10000000) {
                printf("Memory Limit Exceed: I don\'t know why you want so big "
                       "memories or... more than 1000,0000 bytes!\n");
                aaa = false;
                printf("-----------------------------------\n");
                lk = lk + 3 + (q - 1) / 175;
            }
    ...
    while(nn <= en) {
        if(bp[nn]) {
            ...
            while(1) {
                ...
                if(KEY_DOWN(MOUSE_MOVED)) {
                    got();
                    if(pt.y > (10 + sg3 + (q - 1) / 175) * 16
                       && pt.y < (14 + sg3 + (q - 1) / 175) * 16 && pt.x > 0
                       && pt.x < 48) {
                        int h = (mn + 9) / 10;
                        for(int i = 0; i < h; i++) {
                            for(int j = 0; j < 10; j++) {
                    ...
                    if(pt.y > (10 + sg3 + (q - 1) / 175) * 16
                       && pt.y < (14 + sg3 + (q - 1) / 175) * 16 && pt.x > 48
                       && pt.x < 96) {
                        printf("Continue...\n");
                        Sleep(3000);
                        Goto(7 + sg3, 0);
        ...
        if(lo[nn] == '<')
            ...
            while(by[now] != 0) {
                ...
                if(cs > 1000000) {
                    Goto(7, 0);
                    printf("\nRuntime Error: soo many loops or... more than "
                           "100,0000 times!\n");
                    ++ssc;
                    return;
                }
    ...
    while(1) {
        if(kbhit()) {
            ...
            if(hhhh == 'A' || hhhh == 'a') {
                ...
                colset(12);
                if(bp[qw])
                    printf("Now there is a breakpoint.");
                else
                    printf("Now there is not a breakpoint.");
                printf("Press or click to continue...");
                while(1) {
                    if(kbhit() || KEY_DOWN(MOUSE_MOVED))
                        return;
                }
                break;
            }
        ...
        if(KEY_DOWN(MOUSE_MOVED)) {
            Sleep(100);
            got();
            if((pt.y > 0 + (4 + (q - 1) / 175) * 16
                && pt.y < 64 + (4 + (q - 1) / 175) * 16 && pt.x > 48
                && pt.x < 96)
               || (pt.y > 64 + (4 + (q - 1) / 175) * 16
                   && pt.y < 128 + (4 + (q - 1) / 175) * 16 && pt.x > 0
                   && pt.x < 48)
               || (pt.y > 64 + (4 + (q - 1) / 175) * 16
                   && pt.y < 128 + (4 + (q - 1) / 175) * 16 && pt.x > 96
                   && pt.x < 136)
               || (pt.y > 128 + (4 + (q - 1) / 175) * 16
                   && pt.y < 192 + (4 + (q - 1) / 175) * 16 && pt.x > 48
                   && pt.x < 96)) {
                Goto((qw - 1) / 175 + 3, (qw - 1) % 175);
                colset(15);
                if(bp[qw])
...
void mdf() {
    ...
    while(1) {
        ...
        if(KEY_DOWN(MOUSE_MOVED)) {
            got();
            if(pt.y > 0 && pt.y < 64 && pt.x > 0 && pt.x < 48)
                return;
        }
    }
}
...
void im(int be, int en) {
    ...
    while(nn <= en && aaaa) {
        ...
        if(KEY_DOWN(MOUSE_MOVED)) {
            ...
            if(pt.y > 0 && pt.y < 64 && pt.x > 808 && pt.x < 856)
                return;
        }
        if(kbhit()) {
            char haha = getch();
...
void imt() {
    ...
    colset(15);
    system("cls");
    printf("You will use this when you use less memorys , ouput , code and "
           "time.\n");
    printf("Are you ");
    colset(12);
    printf("SURE");
    ...
    do {
        ...
        got();
            ...
            && !(KEY_DOWN(MOUSE_MOVED) && pt.y > 32 && pt.y < 96 && pt.x > 0
                 && pt.x < 72));
    if(KEY_DOWN(MOUSE_MOVED)) {
        if(pt.x > 40 && pt.x < 72)
            return;
...
void hlp() {
    system("cls");
    printf("Brainfuck is a minimal computer language created by Urban Muller "
           "in 1993 \n");
    printf("> pointer plus one \n< pointer minus one \n+ pointer to byte plus "
           "one \n- pointer to byte minus one \n");
    printf(". Output pointer pointing unit content (ASCII code) \n, input "
           "content to pointer pointing unit (ASCII code) \n");
    printf("[ jumps back to the next instruction of the corresponding ] "
           "instruction if the cell value the pointer points to is zero \n");
    printf("] jumps forward to the corresponding [ next instruction of "
           "instruction if the cell value that the pointer points to is not "
           "zero.\n");
    printf("+-----+\n|Close|\n|    C|\n+-----+");
    while(1) {
        Sleep(50);
            ...
            if(KEY_DOWN(MOUSE_MOVED) && pan(pt.y, pt.x)) {
                if(pan(pt.y, pt.x) == 1) {
                    ...
                    while(1) {
                        ...
                        if(KEY_DOWN(MOUSE_MOVED)) {
                            if(pt.y > lk * 16 && pt.y < lk * 16 + 64 && pt.x > 0
                               && pt.x < 56)
                                break;
                        }
                    }
